...
export default function LoginPage() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    // Программная валидация полей для обхода скрытых браузерных bubbles в overflow-hidden карточке
    if (!email || !password) {
    ...
    try {
      // Инициализируем вход через официальный SDK Clerk
      const result = await signIn.create({
        identifier: email,
        password: password,
      });

      // Если авторизация завершена успешно, активируем сессию на клиенте
      if (result.status === "complete") {
        ...
        setIsLoading(false);
        setError("Требуются дополнительные шаги для подтверждения аккаунта.");
      }
    } catch (err: any) {
      setIsLoading(false);
      // Безопасно выводим ошибку, полученную от серверов Clerk
      setError(err.errors?.[0]?.message || err.message || "Неверный логин или пароль.");
    }
  };
  ...
  return (
    ...
    >
      ...
      <GlassCard className="p-8 border-white/5 bg-zinc-950/40 relative overflow-hidden">
        {/* Анимированный полноэкранный оверлей загрузки */}
        <AnimatePresence>
          {isLoading && (
            <motion.div
              initial={{ opacity: 0 }}
        ...
        <form onSubmit={handleLogin} className="space-y-5">
          ...
          <Button type="submit" isLoading={isLoading} disabled={!isLoaded} className="w-full">
            Войти в систему
          </Button>
        </form>
        ...
        {/* Кнопки авторизации через социальные сети */}
      ...
      {/* Ссылка на переключение на страницу регистрации */}
